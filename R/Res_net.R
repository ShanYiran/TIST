...
#'
#' }
Dropout_net_analysis <- function(SC_obj,
                                   Spot_manifest,
                                   savePath,
                                   spaceFile,
                                   Maskfile,
  ...
  dropout_matrix <- expr_matrix
  space <- Spot_manifest
  rownames(space) <- Spot_manifest$barcode
  len <- dim(dropout_matrix)[1]
  id_sample <- sample(x = 1:(dim(dropout_matrix)[1]*dim(dropout_matrix)[2]),size = round(dim(dropout_matrix)[1]*dim(dropout_matrix)[2]*dropout_rate))
  for(i in 1:length(id_sample)){
    id <- id_sample[i]
    row <- ceiling(id/len)
    col <- id%%len
    if(col==0) col = len
    dropout_matrix[col,row] = 0
  }

  dir.create(paste0(savePath,"dropout","_d",dropout_rate))
  savePath_ <- paste0(savePath,"dropout","_d",dropout_rate,"/")
  saveRDS(dropout_matrix,paste0(savePath_,"dropout_matrix.RDS"))


  if(F){
    id <-dropout_matrix["Spink8",]
    #id <- Dc
    id <- abs(id)
    ...
    p <- ggplot(pltdat, aes(x = x, y = y, color = id)) + geom_point(size = 3)+ scale_color_gradientn(colours = pal(5)) +scale_x_discrete(expand = c(xpand, ypand)) + scale_y_discrete(expand = c(xpand, ypand)) + coord_equal() + 
      ...
      theme_bw()
    ggsave(filename = paste0(savePath_,"Spink8_final.png"), p,
    ...
    p <- ggplot(pltdat, aes(x = x, y = y, color = id)) + geom_point(size = 3)+ scale_color_gradientn(colours = pal(5)) +scale_x_discrete(expand = c(xpand, ypand)) + scale_y_discrete(expand = c(xpand, ypand)) + coord_equal() + 
      ...
      # labs(title = colnames(pd)[igene+2], x = NULL, y = NULL)+
      theme_bw()
    ggsave(filename = paste0(savePath_, "Spink8_before.png"), p,
           width = 8, height = 7, dpi = 500)
